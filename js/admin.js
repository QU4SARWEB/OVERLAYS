(function(){
  ...
  function loadFromSupabase(id){
    if(!_sb||!id)return
    _sb.from('matches').select('*').eq('id',id).single().then(function(r){
      if(r.data){
        var m=r.data
        var d={t1n:m.team1_name||D.t1n,t1s:m.team1_score||0,t1l:m.team1_logo||D.t1l,t2n:m.team2_name||D.t2n,t2s:m.team2_score||0,t2l:m.team2_logo||D.t2l,mt:m.title||D.mt,ms:m.serie||D.ms,mm:m.map||D.mm,ww:m.wing_width||D.ww}
        apply(d);save(d);_log('Partida cargada desde Supabase')
      }
    })
  }
  ...
  function loadMatchesList(){
    if(!_sb)return
    _sb.from('matches').select('*').order('created_at',{ascending:false}).then(function(r){
      if(r.error||!r.data)return
      var el=$('pastMatches')
      el.innerHTML=''
      r.data.forEach(function(m){
        var b=document.createElement('button')
        b.textContent=(m.title||'Sin título')+' — '+m.team1_name+' vs '+m.team2_name+' ('+m.serie+')'
        b.style.cssText='display:block;width:100%;text-align:left;padding:8px 12px;margin-bottom:4px;background:rgba(107,47,160,0.08);border:1px solid rgba(107,47,160,0.12);border-radius:8px;color:rgba(255,255,255,0.6);font-size:11px;cursor:pointer;transition:all .2s;font-family:inherit'
        b.addEventListener('mouseenter',function(){this.style.background='rgba(107,47,160,0.2)';this.style.color='#fff'})
        b.addEventListener('mouseleave',function(){this.style.background='rgba(107,47,160,0.08)';this.style.color='rgba(255,255,255,0.6)'})
        b.addEventListener('click',function(){loadFromSupabase(m.id)})
        el.appendChild(b)
      })
    })
  }
  ...
  document.addEventListener('DOMContentLoaded',function(){
    ...
    try{if(typeof supabase!=='undefined'){
      ...
      _ch.subscribe(function(s){
        if(s==='SUBSCRIBED'){_log('Supabase conectado');loadMatchesList();broadcast(gs())}
      })
    }}catch(e){}
    ...
    $('shareBtn').addEventListener('click',function(){
      var d=gs()
      var url='https://qu4sarweb.github.io/OVERLAYS/overlay.html?t1n='+encodeURIComponent(d.t1n)+'&t1s='+d.t1s+'&t2n='+encodeURIComponent(d.t2n)+'&t2s='+d.t2s+'&mt='+encodeURIComponent(d.mt)+'&ms='+encodeURIComponent(d.ms)+'&mm='+encodeURIComponent(d.mm)+'&ww='+d.ww
